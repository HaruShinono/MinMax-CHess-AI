    ...
    for row in range(len(gs.board)):
        for col in range(len(gs.board[row])):
            ...
            if square != "--":
                ...
                if square[1] == "p":
                    piecePositionScore = pawnScore[row][col]
                elif square[1] == "N":
...
def endGame(gs):
    ...
    return (not whiteQueen and not blackQueen) or \
        (whiteQueen and not blackQueen and whiteRooks + whiteBishops + whiteKnights <= 1) or \
        (blackQueen and not whiteQueen and blackRooks + blackBishops + blackKnights <= 1)